 ...
 *
 * @Title: ShadowDrawable.java
 * @Prject: MTQBusFreighthelper
 * @Package: com.mtq.bus.freighthelper.ui.customview
 * @Description: 自定义投影效果控件
 * @author: zhaoqy
 * @date: 2017年7月10日 上午9:03:39
 * @version: V1.0
 */
...
public class ShadowDrawable extends Drawable {
	...
	public ShadowDrawable(int bgColor, float shadowRange, float shadowDx,
			float shadowDy, int shadowColor) {
		paint = new Paint();
	...
	protected void onBoundsChange(Rect bounds) {
		...
		if (bounds.right - bounds.left > 0 && bounds.bottom - bounds.top > 0) {
			...
			drawRect = new RectF(offsetLeft, offsetTop, width - offsetRight,
					height - offsetBottom);
			invalidateSelf();
		}
	}
	...
	public static class Builder {
		...
		public ShadowDrawable create() {
			...
			final ShadowDrawable shadowViewDrawable = new ShadowDrawable(
					bgColor, shadowRange, shadowDx, shadowDy, shadowColor);

			view.setPadding(view.getPaddingLeft() + (int) offsetLeft,
					view.getPaddingTop() + (int) offsetTop,
					view.getPaddingRight() + (int) offsetRight,
					view.getPaddingBottom() + (int) offsetBottom);
